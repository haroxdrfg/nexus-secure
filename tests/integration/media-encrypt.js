...
function validateMedia(mediaBuffer, mimeType) {
  ...
  if (mediaBuffer.length > MAX_MEDIA_SIZE_BYTES) {
    throw new Error('Media trop volumineux (max ' + (MAX_MEDIA_SIZE_BYTES / 1024 / 1024) + ' Mo)');
  }
...
function encryptMedia(mediaBuffer, aesKey) {
  if (!Buffer.isBuffer(aesKey) || aesKey.length !== 32) {
    throw new Error('Cle AES invalide (32 octets requis)');
  }
  const chunks = [];
  let offset = 0;
  ...
  return { chunks, totalSize: mediaBuffer.length, chunkCount: chunks.length };
}
...
function decryptMedia(encryptedMedia, aesKey) {
  ...
  for (const chunk of sorted) {
    ...
    try {
      ...
      throw new Error('Echec de dechiffrement du chunk ' + chunk.index + ' - donnees corrompues ou falsifiees');
    }
  }
...
function recipientDecryptMedia(envelope, recipientPrivKeyPEM) {
  ...
  if (!crypto.timingSafeEqual(Buffer.from(metaHmac, 'base64'), Buffer.from(expectedHmac, 'base64'))) {
    throw new Error('HMAC invalide - enveloppe corrompue ou falsifiee');
  }
