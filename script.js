...
function disconnect(silent=false) {
  ...
  const btnMediaD = document.getElementById('btn-media');
  if (btnMediaD) btnMediaD.disabled = true;
  const fpWarning = document.getElementById('fp-warning');
...
async function sendMessage() {
  ...
  if (!S.activePair.fpVerified) {
    notify('Empreinte non verifiee. Verifiez avec votre pair.', 'warn');
  }
...
const MEDIA_MIME_ALLOWED = new Set([
  'image/jpeg', 'image/png', 'image/gif', 'image/webp',
  'video/mp4', 'video/webm', 'video/quicktime'
]);
const MEDIA_MAX_SIZE = 100 * 1024 * 1024;
const MEDIA_CHUNK = 5 * 1024 * 1024;

let pendingMediaFile = null;

function previewMedia(file) {
  if (!S.activePair) { notify('Pas de canal actif.', 'err'); return; }
  if (!MEDIA_MIME_ALLOWED.has(file.type)) {
    notify('Type non autorise: ' + file.type, 'err');
    return;
  }
  if (file.size > MEDIA_MAX_SIZE) {
    notify('Fichier trop volumineux (max 100 Mo).', 'err');
    return;
  }
  if (file.size === 0) {
    notify('Fichier vide.', 'err');
    return;
  }
  ...
  if (file.type.startsWith('video/')) {
    content.innerHTML = '<video src="' + url + '" style="max-height:120px;max-width:200px;border-radius:6px;" muted></video>'
      + '<span style="margin-left:8px;font-size:12px;color:var(--text-2);">' + escapeHtml(file.name) + ' (' + (file.size / 1024).toFixed(0) + ' Ko)</span>';
  } else {
    content.innerHTML = '<img src="' + url + '" style="max-height:120px;max-width:200px;border-radius:6px;">'
      + '<span style="margin-left:8px;font-size:12px;color:var(--text-2);">' + escapeHtml(file.name) + ' (' + (file.size / 1024).toFixed(0) + ' Ko)</span>';
  }
...
async function encryptMediaClient(buffer, sharedKey) {
  ...
  while (offset < buffer.byteLength) {
    const slice = buffer.slice(offset, offset + MEDIA_CHUNK);
    const iv = crypto.getRandomValues(new Uint8Array(12));
  ...
  return { chunks, totalSize: buffer.byteLength, chunkCount: chunks.length };
}

async function decryptMediaClient(encrypted, sharedKey) {
...
async function sendMedia(file) {
  ...
  notify('Chiffrement du media...', 'warn');
  const buffer = await file.arrayBuffer();
  const encrypted = await encryptMediaClient(buffer, S.activePair.sharedKey);
  const mediaId = genId(16);
  const sentAt = now();
  ...
  const sig = await signData(S.mySignKeyPair.privateKey, refPayload);

  const apiEnvelope = {
    mimeType: file.type,
    salt: 'client-e2e',
    ephemeralPub: 'client-e2e',
    metaHmac: 'client-e2e',
    encrypted
  };

  try {
    const resp = await fetch('/api/media/store', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ mediaId, envelope: apiEnvelope })
    });
    if (!resp.ok) throw new Error('Stockage echoue');
  } catch (e) {
    notify('Erreur envoi media: ' + e.message, 'err');
    return;
  }

  await store(`media-ref:${cid}:${mediaId}`, { ...refPayload, sig });

  const isVideo = file.type.startsWith('video/');
  const blob = new Blob([buffer], { type: file.type });
  const url = URL.createObjectURL(blob);
  renderMediaMessage({ id: mediaId, dir: 'out', sentAt, url, mimeType: file.type, fileName: file.name, isVideo });
  notify('Media envoye (chiffre).');
}

async function pollMedia() {
  if (!S.activePair) return;
  const cid = convId(S.myId, S.activePair.peerId);
  const keys = await listKeys(`media-ref:${cid}:`);

  for (const k of keys) {
    const ref = await load(k);
    if (!ref) continue;
    if (ref.from === S.myId) continue;
    if (S.seenNonces.has(ref.nonce)) { await del(k); continue; }

    if (ref.to !== S.myId || ref.cid !== cid) { await del(k); continue; }

    const { sig, ...payload } = ref;
    let sigValid = false;
    try {
      sigValid = await verifySignature(S.activePair.peerSignKey, payload, sig);
    } catch {}
    if (!sigValid) {
      addSysMsg('Media avec signature invalide rejete.', 'er');
      await del(k);
      continue;
    }

    S.seenNonces.add(ref.nonce);
    persistNonces();

    try {
      const resp = await fetch(`/api/media/retrieve/${ref.mediaId}`);
      if (!resp.ok) { await del(k); continue; }
      const data = await resp.json();
      const env = data.envelope;
      const decrypted = await decryptMediaClient(env.encrypted, S.activePair.sharedKey);
      const isVideo = ref.mimeType.startsWith('video/');
      const blob = new Blob([decrypted], { type: ref.mimeType });
      const url = URL.createObjectURL(blob);
      renderMediaMessage({ id: ref.mediaId, dir: 'inc', sentAt: ref.sentAt, url, mimeType: ref.mimeType, fileName: ref.fileName, isVideo });
    } catch (e) {
      addSysMsg('Impossible de dechiffrer le media.', 'er');
    }
    await del(k);
  }
}

function renderMediaMessage({ id, dir, sentAt, url, mimeType, fileName, isVideo }) {
  S.messages.push({ id, dir, expiresAt: sentAt + 10 * 60 * 1000 });
  const container = document.getElementById('msgs');
  document.getElementById('no-peer-view').style.display = 'none';
  ...
  let mediaHtml;
  if (isVideo) {
    mediaHtml = `<video src="${url}" controls class="media-preview" style="max-width:320px;max-height:240px;border-radius:6px;"></video>`;
  } else {
    mediaHtml = `<img src="${url}" class="media-preview" style="max-width:320px;max-height:320px;border-radius:6px;cursor:pointer;" onclick="window.open('${url}','_blank')">`;
  }

  el.innerHTML = `
    <div class="msg-bubble">${mediaHtml}<div style="font-size:11px;color:var(--text-3);margin-top:4px;">${escapeHtml(fileName || 'media')}</div></div>
    <div class="msg-meta">
      <span>${timeStr}</span>
      <span class="msg-sig ok">SIGN OK</span>
      <span class="badge badge-blue" style="font-size:9px">E2E</span>
    </div>
  `;
